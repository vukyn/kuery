...
✅ **Ledger entry, 2026-09-11 — `http/health` shipped to BOTH twins, no gap.** The new deployment-verification handler landed in `http/fiber` and `http/fiberv3` in the same PR (kuery #58, v1.61.0). It is the first feature since the drift above that did not open a new one, and the reason is structural rather than diligence: all the logic lives in `http/health`, a framework-free package, so each twin is a three-line wrapper differing only by `*fiber.Ctx` vs `fiber.Ctx`. There is nothing to drift. `http/fiberv3` also got its **first test file** here, which chips at the zero-coverage problem noted above for `ctxv3`.

**How to apply:** when adding a Fiber-coupled feature, put the logic in a framework-free package and keep the twin a thin adapter — that is what makes porting free instead of a judgement call each time.

✅ **Ledger entry, 2026-09-18 — `apikey`/`apikeyv3` shipped to BOTH twins in the same PR, no gap.** Inbound API-key auth landed as a full twin pair rather than a v2-only package, so nothing is lost on migration. It deliberately does NOT take the thin-adapter shape recommended above: its framework-free parts (`Key`, `Config`, `ParseKeys`) are part of the **exported surface**, so a shared core would have to be a third public package (kuery has no `internal/`) and a v3 consumer importing it for those types would pull `fiber/v2` straight back into its build graph — the one thing the twins exist to prevent. The two files are instead byte-parallel modulo the mechanical transform, and so are their tests.

**How to apply:** when a Fiber-coupled package's framework-free parts are its exported TYPES, extracting a core re-exports the framework dependency with them — copy the twin and use a mechanical diff as the parity check: `diff <(sed -e 's|/v3|/v2|;s|fiberv3|fiber|;s|c fiber\.Ctx|c *fiber.Ctx|' apikeyv3/apikey.go) apikey/apikey.go` must be empty. Run it after ANY edit to either file; that is what stops this pair drifting the way `auth`/`authv3` did.
