...
func (d *Document) Summary(pairs ...[2]string) {
	for _, p := range pairs {
		// Label right-aligned across the left 8/12 (wide enough to keep multi-word
		// labels like "Số nhật ký chăm sóc" on one line); value right-aligned in
		// the trailing 4/12 so numeric values line up as a clean right column.
		d.m.AddRow(7,
