...
| `apikey` / `apikeyv3` | Fiber v2 / v3 | inbound API-key auth — the sibling of `auth` for machine callers (cron runner, internal service, device) that have no token to refresh. Compares the **SHA-256 digest** of the presented header, never the raw key: both operands are then always 64 chars, which is what makes `subtle.ConstantTimeCompare` meaningful — a raw compare returns on the first length mismatch and leaks the secret's length. **Every** configured key is visited, no early exit, so timing cannot reveal which slot matched (several keys = the rotation path). **Empty `Keys` rejects every request** — fails closed. Matched key's `Label` goes into Fiber locals (`Label`/`LabelFrom`); a missing header and a wrong key are indistinguishable in the response. `ParseKeys` parses one envconfig string (`hash:label,hash`) and rejects non-64-hex, duplicate and over-max entries **without echoing the offending value**. Retains nothing request-derived, so no `strings.Clone` — see the file's comment before "fixing" that |
...
Cross-package dependency chains to keep in mind when editing: `jwt → claims → cryp`, `ctx → claims`, `bun/query → http/base`, `bun/hooks`/`graceful` → `log`. `http/errors → http/base` (for `Forward`), `http/ratelimit → http/{errors,fiber}` + `log` + `text`, `apikey`/`apikeyv3` → `cryp` + `http/{errors,fiber}` / `http/{errors,fiberv3}`.
